...
if (isset($_GET['export'])) {
    ...
    while ($row = $res->fetch_assoc()) {
        fputcsv($out, [
            $row['description'], $row['serial'], $row['make'], $row['model'],
            $row['location'], $row['username'], $row['password'],
            $row['ipaddress'], $row['note']
        ]);
    }
    fclose($out);
    exit;
...
<div class="container mt-4">
  ...
  <form method="POST" class="row g-2 mb-4">
    ...
    <div class="col-md"><input class="form-control" name="description" placeholder="Description" value="<?= $edit_record['description'] ?? '' ?>" required></div>
    <div class="col-md"><input class="form-control" name="serial" placeholder="Serial" value="<?= $edit_record['serial'] ?? '' ?>"></div>
    <div class="col-md"><input class="form-control" name="make" placeholder="Make" value="<?= $edit_record['make'] ?? '' ?>"></div>
    <div class="col-md"><input class="form-control" name="model" placeholder="Model" value="<?= $edit_record['model'] ?? '' ?>"></div>
    <div class="col-md"><input class="form-control" name="location" placeholder="Location" value="<?= $edit_record['location'] ?? '' ?>"></div>
    <div class="col-md"><input class="form-control" name="username" placeholder="Username" value="<?= $edit_record['username'] ?? '' ?>"></div>
    <div class="col-md"><input class="form-control" name="password" placeholder="Password" value="<?= $edit_record['password'] ?? '' ?>"></div>
    <div class="col-md"><input class="form-control" name="ipaddress" placeholder="IP Address" value="<?= $edit_record['ipaddress'] ?? '' ?>"></div>
    <div class="col-md"><input class="form-control" name="note" placeholder="Note" value="<?= $edit_record['note'] ?? '' ?>"></div>
  ...
  <table class="table table-bordered">
    <thead class="table-light">
      <tr>
        <th>Description</th><th>Serial</th><th>Make</th><th>Model</th><th>Location</th>
        <th>Username</th><th>Password</th><th>IP Address</th><th>Note</th><th>Actions</th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <a href="?client_id=<?= $client_id ?>&delete=<?= $r['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this entry?')">🗑️</a>
        </td>
      </tr>
      <?php endwhile; ?>
    </tbody>
